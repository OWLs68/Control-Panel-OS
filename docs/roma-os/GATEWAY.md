...
### 12.2. Ще не перевірено — тільки на Mac і iPhone

Реального Hermes у хмарній сесії немає. Справжній ланцюг
Hermes → DeepSeek → `mcp__gbrain__recall` → `VIOLET-624` перевірив лише Роман
своїм Mac-клієнтом; через gateway його підтверджує `curl` з §9 і чекліст
`STAGE-1.md §6.5` (Live mode, повідомлення через Tailscale, одна відповідь,
маркер із GBrain, зупинений Hermes → зрозуміла помилка, Памʼять наживо далі
працює).
