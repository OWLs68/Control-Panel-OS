...
### 2. Хвости попередніх сесій — знайти і назвати, не вливати
...
`/start` — read-only: хвости лише знаходить і називає; merge чи push — лише
за окремим словом Романа (HOT RULE 3).
...
for b in $(git for-each-ref --format='%(refname:short)' refs/remotes/origin/claude/); do
  ...
  if [ -z "$(git diff --stat origin/main..."$b" -- . ':!docs' ':!CLAUDE.md' ':!.claude' ':!README.md')" ]; then
    echo "DOCS-TAIL: $b"      # лише документи → назвати в звіті
  else
...
Обидва типи хвостів → **лише назвати в звіті** (гілка, коміти, що в них).
Нічого не вливати і не пушити. Якщо docs-хвіст несе новіший блок «ПОТОЧНИЙ
СТАН», прочитати його з тієї гілки (`git show <гілка>:docs/roma-os/HANDOFF.md`)
і сказати, що стан у `main` відстає. Вливання — окреме рішення Романа.
...
2. `Branch: … · HEAD: …` (+ «вже в main»), знайдені хвости, якщо були
3. `CI: ✅/⚠️ …` 4. `Live: ✅/⚠️ …`
